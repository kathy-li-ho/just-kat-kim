...
const path = obj => {
    return obj && 'node' in obj ? obj.node.fluid : {src: transparentPNG};
};
const generateSizes = () => {
...
const Footer = () => {
    const footers = useStaticQuery(graphql`
        query {
            allImageSharp(
                filter: {fluid: {originalName: {regex: "/footer/"}}}
            ) {
                edges {
                    node {
                        fluid(
                            srcSetBreakpoints: [576, 768, 992, 1200, 1600]
                            maxWidth: 1920
                        ) {
                            src
                            srcSet
    ...
    const {src, srcSet} = path(
        footers.filter(edge => path(edge).src.includes(page))[0]
    );
    const imageProps = {
        className: 'footerImg',
        src,
